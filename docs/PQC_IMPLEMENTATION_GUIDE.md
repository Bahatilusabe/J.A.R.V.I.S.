# Post-Quan### Architecture

Components

```text
┌─────────────────────────────────────────────────────────────┐
│                    PQC System Architecture                   │ptography (PQC) Implementation Guide

## Overview

This document describes J.A.R.V.I.S.'s Post-Quantum Cryptographic (PQC) system, which protects against future threats from quantum computers. The system combines:
...
```
┌─────────────────────────────────────────────────────────────┐
│                    PQC System Architecture                   │
├─────────────────────────────────────────────────────────────┤
│                                                               │
│  ┌──────────────────────────────────────────────────────┐   │
│  │  PQC Configuration & Key Management (config.py)       │   │
│  │  - Algorithm selection (Kyber512/768/1024)           │   │
...
│                                                               │
│  ┌──────────────────┐         ┌──────────────────────────┐  │
│  │  Kyber KEM       │         │  Dilithium DSA           │  │
│  │  (kyber.py)      │         │  (dilithium.py)          │  │
│  │                  │         │                          │  │
│  │ · Keypair Gen   │         │ · Keypair Gen           │  │
│  │ · Encapsulate   │         │ · Sign                  │  │
│  │ · Decapsulate   │         │ · Verify                │  │
│  │ · 3 sec levels  │         │ · 3 sec levels          │  │
│  └──────────────────┘         └──────────────────────────┘  │
│                                                               │
│  ┌──────────────────────────────────────────────────────┐   │
│  │  PQC Handshake Protocol (handshake.py)               │   │
│  │  - ClientHello / ServerHello                         │   │
│  │  - Key exchange (Kyber encapsulation)               │   │
...
|-------|-------|-----------|-------------------|----------|
| 1     | Kyber512 | Dilithium2 | ~128 | Testing, non-critical systems |
| 3     | Kyber768 | Dilithium3 | ~192 | Default for general use |
| 5     | Kyber1024 | Dilithium5 | ~256 | High-security critical systems |
...
# {
#   "kem_public": "base64_encoded_public_key",
#   "sig_public": "base64_encoded_public_key",
#   "kem_key_id": "kem-abc123...",
#   "sig_key_id": "sig-def456...",
#   "kem_algorithm": "Kyber768",
#   "sig_algorithm": "Dilithium3"
# }
```

### 2. Kyber KEM (Key Exchange)
...
# Both have the same shared secret, never transmitted!
```

### 3. Dilithium Signatures (Authentication)
...
# Parse ClientKeyExchange (JSON format)
...
# session is now established with shared secret and derived keys
...
print(f"Client write key: {session.client_write_key.hex()}")
print(f"Server write key: {session.server_write_key.hex()}")
```
...
### 5. Hybrid Mode (Kyber + Classical ECDH)
...
# shared_secret is XOR of both Kyber and ECDH secrets
# Even if either is broken, the other protects the secret
```

## Integration with FastAPI Routes

### Example: PQC Session Establishment Route

```python
# backend/api/routes/pqc_routes.py
...
# Store active handshakes (in production, use Redis/DB)
active_handshakes = {}

class ClientHelloRequest(BaseModel):
    data: str  # JSON-encoded ClientHello

class ServerHelloResponse(BaseModel):
    data: str  # JSON-encoded ServerHello

@router.post("/pqc/handshake/hello")
async def pqc_client_hello(request: ClientHelloRequest):
    """Accept ClientHello and respond with ServerHello."""
    try:
        client_hello = ClientHello.from_json(request.data)
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Invalid ClientHello: {e}")
    ...
    # Store for later
    handshake_id = server_hello.timestamp  # or UUID
    active_handshakes[handshake_id] = server
    
    return ServerHelloResponse(data=server_hello.to_json())

@router.post("/pqc/handshake/key-exchange")
async def pqc_key_exchange(
    handshake_id: str,
    client_key_exchange_data: dict = Body(...)
):
    """Accept ClientKeyExchange and establish session."""
    server = active_handshakes.get(handshake_id)
    if not server:
    ...
    try:
        ...
        client_key_exchange = ClientKeyExchange(
            client_ephemeral_public_key=base64.b64decode(
                client_key_exchange_data["client_ephemeral_public_key"]
            ),
            encapsulated_secret=base64.b64decode(
                client_key_exchange_data["encapsulated_secret"]
            ),
            client_certificate=base64.b64decode(
                client_key_exchange_data["client_certificate"]
            ),
        )
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Invalid KeyExchange: {e}")
    
    session, state = server.process_client_key_exchange(client_key_exchange)
    
    # Store session (in production, use Redis/DB)
    from backend.core.pqc_session_store import store_session
...
Then register in `backend/api/server.py`:
...
- Old key ID is tracked for decryption of old ciphertexts

### Handshake Timeout

- Default: 30 seconds (configurable via `PQC_HANDSHAKE_TIMEOUT`)
- Prevents indefinite resource consumption from stalled handshakes
- Raises `HandshakeState.FAILED` on timeout
...
old_id = km.kem_key_id
km.rotate_kem_key(reason="test")

assert km.kem_key_id != old_id
...
km = get_key_manager()
km.generate_kem_keypair()
# Output: 2024-01-15 10:30:00 - backend.core.pqcrypto.config - INFO - Generated new Kyber keypair: kem-abc123...
```
...
|-----------|-----------|-----------|-----------|
| Dilithium2 | 1,312 bytes | 2,544 bytes | 2,420 bytes |
| Dilithium3 | 1,952 bytes | 4,016 bytes | 3,293 bytes |
| Dilithium5 | 2,592 bytes | 4,880 bytes | 4,595 bytes |
...
1. **PSK Mode** - Pre-shared key mode for session resumption
2. **0-RTT** - Zero round-trip time early data
3. **Post-Handshake Auth** - Mid-connection authentication refresh
4. **Key Compromise Impersonation (KCI) Resistance**
5. **Distributed Key Generation** - Threshold cryptography for shared keys
6. **Hardware Integration** - Full HSM support with PKCS#11
